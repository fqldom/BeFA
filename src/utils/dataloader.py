...
class TrainDataLoader(AbstractDataLoader):
    ...
    def __init__(self, config, dataset, batch_size=1, shuffle=False):
        ...
        # self.all_items = self.product_info[self.dataset.iid_field].unique().tolist()
        ...
        # load text info
        # self.load_item_text_info(config['text_file_path'])
        # # load images and text
        # self.load_images(config['vision_file_path'])
        # # self.load_text(self.product_info)
        #
        # prompt = config['prompt']
        # self.text_list = self.product_info['prod_info'].tolist()
        # self.text_list = [prompt + text for text in self.text_list]
        #
        # for text in self.text_list:
        #     if len(text) > 20:
        #         truncated_string = text[:20]
        #         self.text_list[self.text_list.index(text)] = truncated_string
        #
        ...
        #     ToTensor(),
        # ])
        # dataset = CustomDataset(self.image_path_list, self.text_list, transform=image_transform)
        # self.dataloader = DataLoader(dataset, batch_size=self.load_batch_size, shuffle=False)
    ...
    def load_item_text_info(self, file_name):
        text_file = os.path.join(self.dataset.dataset_path, file_name)
        self.product_info = pd.read_csv(text_file)
    def load_images(self,file_dir):
        image_folder = os.path.join(self.dataset.dataset_path, file_dir)
        image_path_list = []
        for image_id in self.all_items:
    ...
    def _create_sparse_matrix(self, df_feat, source_field, target_field, form='coo', value_field=None):
        ...
        # mat = coo_matrix((data, (src, tgt)), shape=(src.max()+1, tgt.max()+1))
        if form == 'coo':
            return mat
        elif form == 'csr':
            return mat.tocsr()
